...
from em_hsd import spine_bootstrap  # noqa: F401 — path setup before mechanism import
...
class EmHsdConfig:
    ...
    triage_dp: TriageDpSettings = field(default_factory=TriageDpSettings)
    ...
    def is_triage_dp_mode(self) -> bool:
        return self.triage_dp.enabled
...
def _parse_triage_dp(d: dict) -> TriageDpSettings:
    td = d.get("triage_dp", {}) or {}
    return TriageDpSettings(
        enabled=bool(td.get("enabled", False)),
        layer1=dict(td.get("layer1", {})),
        layer2=dict(td.get("layer2", {})),
        layer3=dict(td.get("layer3", {})),
        layer4=dict(td.get("layer4", {})),
    )
...
def load_em_hsd_config(path: str) -> EmHsdConfig:
    ...
    return EmHsdConfig(
        ...
        triage_dp=_parse_triage_dp(data),
    )
