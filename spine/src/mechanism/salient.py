...
def classify_word(surface: str) -> str:    s = surface.strip().lower()
    if not s:
        return "content"
    if s.isdigit():
        return "number"
...
def classify_segment(seg: Segment) -> str:    if seg.kind in (URL, MENTION, HASHTAG):
...
def is_rewritable(token_class: str) -> bool:    return token_class == "content"
